...
def generate_humid(num):
    partition_id = num % partition
    for index in range(len(df)):
        try:
            base_humid = df['Humidity'].iloc[index]

            if num == 0:
                humid_value = base_humid
            else:
                humid_value = base_humid + random.uniform(-0.001, 0.001)
...
if __name__ == '__main__':
    ...
    for i in range(_thread):
        t = threading.Thread(target=generate_humid, args=(i,))
        t.start()
        thread_lst.append(t)
